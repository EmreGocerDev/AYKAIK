...
// GlassCard'ı artık burada kullanmıyoruz, stilleri doğrudan vereceğiz.
...
export default function AiAssistant() {
  ...
  const { tintValue, blurPx, borderRadiusPx } = useSettings();
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok || !response.body) { throw new Error(`API hatası: ${response.statusText}`); }
  ...
  return (
    <div
      style={{
        ...
        // borderRadius'ı koşullu olarak uygula
        borderRadius: typeof window !== 'undefined' && window.innerWidth < 768 ? '0px' : `${borderRadiusPx}px`
      }}
      className={`fixed z-50 border border-white/10 shadow-lg flex flex-col 
                 inset-0 rounded-none
                 md:inset-auto md:bottom-6 md:right-6 md:h-[70vh] md:w-full md:max-w-md md:rounded-2xl
                 ${isAnimatingOut ? 'animate-modal-close' : 'animate-modal-open'}`}
    >
      {/* BAŞLIK BÖLÜMÜ: flex-shrink-0 sayesinde sabit kalır */}
      ...
      {/* MESAJ ALANI: flex-1 ve overflow-y-auto sayesinde bu alan dolar ve kendi içinde kayar */}
      ...
      {/* FORM BÖLÜMÜ: flex-shrink-0 sayesinde sabit kalır */}
