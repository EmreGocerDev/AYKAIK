...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('leave-request-notifications')
      .on(
        'postgres_changes',
        ...
        async (payload) => {
           ...
           if (payload.eventType === 'INSERT' && profile.role === 'coordinator') {
            ...
            if (personnel && personnel.region_id === profile.region_id) {
              showNotification('Yeni İzin Talebi', {
                body: `${personnel.full_name} yeni bir izin talebinde bulundu.`,
                icon: '/favicon.ico' 
              });
            }
          }
          ...
          if (payload.eventType === 'UPDATE' && profile.role === 'admin') {
            ...
            if (isApprovedByCoordinator && updatedRequest.status !== oldRequest.status) {
              ...
              showNotification(`Koordinatör İşlemi`, {
                body: `${personnel?.full_name || 'Bir personelin'} izin talebi koordinatör tarafından ${statusText}.`,
                icon: '/favicon.ico'
              });
            }
          }
